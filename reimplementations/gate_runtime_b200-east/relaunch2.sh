...
for arm in T C1 C2 C3; do
  ...
  mv "$ROOT/runs/$arm" "$ROOT/runs/${arm}_lr2e4_tokennorm_collapsed" 2>/dev/null
done
sleep 5
for arm in T C1 C2 C3; do
  ...
  setsid nohup python "$ROOT/code/train_arm.py" \
    ...
    --group-size 8 --prompts-per-step 6 --cap 8192 --max-len 8192 --lr 1e-4 \
    --token-budget 0 --max-steps 400 --max-hours "${MAX_HOURS:-3.6}" \
    --concurrency 48 --grad-ckpt 1 --mb-tokens 16384 --truncated wrong --ckpt-every 10 --loss-norm sequence \
