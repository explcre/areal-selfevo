...
class SeedPoolExhausted(RuntimeError):
    """Raised when the seed pool cannot supply a full iteration. Never downgraded."""
...
def seed_tasks(pool_tasks, used, rng, n):
    ...
    if len(avail) < n:
        # NOT `return []`. An empty list here reaches the caller as `len(accepted) < 2` and
        # logs "only 0 task(s) accepted", which is the SAME message a genuine proposer failure
        # produces. In the HON/HOFF runs that collapsed the two: 31 iterations reported a
        # proposer that had never been called. Exhaustion is a supply fact and gets its own
        # exception and its own refusal reason.
        raise SeedPoolExhausted(
            "seed pool exhausted: %d of %d tasks used, %d remain, %d needed per iteration"
            % (len(used), len(pool_tasks), len(avail), n))
...
def main() -> int:
    ...
    # PREFLIGHT, before a single weight is loaded. The HON/HOFF pair burned two machine-hours
    # discovering at iteration 14 that a 59-task pool cannot supply 45 iterations at 4 tasks
    # each. This check costs microseconds and is the whole lesson of that run.
    need = a.seed_iters * a.tasks_per_iter
    if len(tasks_pool) < need:
        raise SystemExit(
            "PREFLIGHT FAILED: seed pool holds %d tasks but --seed-iters %d x "
            "--tasks-per-iter %d needs %d. Either lower --seed-iters (the proposer then takes "
            "over, which is what the stage banner says this run intends), lower --max-iters, "
            "or enlarge the pool. Refusing to start."
            % (len(tasks_pool), a.seed_iters, a.tasks_per_iter, need))
    print("[orn] preflight OK: seed pool %d tasks >= %d needed for %d warmup iterations"
          % (len(tasks_pool), need, a.seed_iters), flush=True)
    ...
    while it < a.max_iters and (time.time() - t0) < a.max_hours * 3600:
        ...
        if seeded:
            ...
            except SeedPoolExhausted as e:
                stats["refusals"]["seed_pool_exhausted"] = 1
                stats["seed_pool_exhausted_detail"] = str(e)
                logf.write(json.dumps(stats) + "\n"); logf.flush()
                print("[orn] iter %d: SEED POOL EXHAUSTED (%s). This is a supply failure, "
                      "not a proposer failure; stopping rather than logging 31 empty "
                      "iterations." % (it, e), flush=True)
                break
