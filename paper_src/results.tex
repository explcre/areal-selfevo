...
\subsection{The 32768 cap is saturated: doubling the budget buys essentially nothing}
\label{sec:capsat}
...
\texttt{Qwen3.8-27B} at $65536$: same model, same grader, only the cap changed.

\begin{table}[t]
\centering\small
\caption{\texttt{Qwen3.8-27B} at $32768$ against the same model at $65536$. Truncation more
than halves on every benchmark while accuracy barely moves. OlympiadBench at $65536$ is still
running and is reported as not yet measured rather than omitted.}
\label{tab:capsat}
\begin{tabular}{lccccc}
\toprule
...
OlympiadBench & 0.7733 & 17.8\% & \multicolumn{2}{c}{not yet measured} & --- \\
\bottomrule
\end{tabular}
\end{table}

MATH-500 moves $+0.004$ at $n=500$, where the standard error is about $0.006$ --- inside
noise. Each AIME set moves by exactly one problem out of thirty, well inside the $\approx 0.1$
jitter recorded at that $n$. The informative quantity is the pair: \textbf{truncation more
than halves while accuracy does not follow}, which is the signature of a budget that was
already sufficient. The generations being cut off at $32768$ were ones that were going to be
graded wrong regardless. For MATH-500 and both AIME sets, therefore, the $32768$ numbers of
Table~\ref{tab:frontier} are measurements rather than lower bounds, and the ranking there can
be stated plainly.

\paragraph{OlympiadBench is the one that is still open.} It is also the one where truncation
at $32768$ was high enough for verbosity to matter ($17.8\%$ against
\texttt{Frontis-MA1-30B}'s $20.0\%$). Its $65536$ measurement is re-running; the first attempt
at that cap is void for the reason given in Sec.~\ref{sec:harness} and is not quoted here or
anywhere else in this paper.

\paragraph{A caveat that belongs with these numbers.} The $65536$ run used concurrency $40$
against the earlier run's $24$, so the batch composition differed between the two columns.
Decoding is greedy and the requests are independent, so no answer depends on which others ran
beside it; but batched matmuls do not associate identically across batch sizes, and a
one-problem AIME difference is exactly the resolution at which that could contribute. This
does not weaken the conclusion, which rests on the accuracy \emph{not} moving while truncation
halved, and not on the sign of $+0.033$.
