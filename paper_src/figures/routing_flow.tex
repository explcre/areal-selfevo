...
\begin{scope}[on background layer]
  \node[draw=black!25, dashed, rounded corners=3pt, fit=(solved)(tea)(skip)(sftt)(sft),
        ...
        RL-silent: 57.4\% of groups, measured}] {};
\end{scope}
\end{tikzpicture}
\caption{%
  ...
  ordinary GRPO cannot learn from, measured at $57.4\%$ of groups. It splits into a half
  where a supervised target is free (the unit's own correct sample) and a half where no
  target exists unless supplied --- and on that second half the harness is the only
  consumer that can use the unit at all.}
\label{fig:routing}
\end{figure}
