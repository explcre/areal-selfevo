#!/usr/bin/env bash
# Launch step0i detached, with a watchdog that reads step0i's own logs.
#
...
#      log pins that maximum at its final value forever. A healthy step0i would have read
...
setsid bash "$H/step0i.sh" > "$RUN/outer.log" 2>&1 < /dev/null &
sleep 3
...
echo "$pgid" > "$RUN/pgid"
echo "step0i trainer PGID=$pgid started $(date -Is)"

# Both logs belong to step0i. Liveness must not depend on one writer, and must never
# depend on a different experiment's log.
setsid bash "$H/watchdog.sh" "$RUN/pgid" 1800 \
    "$RUN/train.log" \
    "$HOME/areal-runs/logs/ubuntu/step0i/t1/main.log" \
