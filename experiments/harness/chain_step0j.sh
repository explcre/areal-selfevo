#!/usr/bin/env bash
# Launch step0j detached, with a watchdog that reads step0j's own logs.
#
...
#      log pins that maximum at its final value forever. A healthy step0j would have read
...
setsid bash "$H/step0j.sh" > "$RUN/outer.log" 2>&1 < /dev/null &
sleep 3
...
echo "$pgid" > "$RUN/pgid"
echo "step0j trainer PGID=$pgid started $(date -Is)"

# Both logs belong to step0j. Liveness must not depend on one writer, and must never
# depend on a different experiment's log.
setsid bash "$H/watchdog.sh" "$RUN/pgid" 1800 \
    "$RUN/train.log" \
    "$HOME/areal-runs/logs/ubuntu/step0j/t1/main.log" \
