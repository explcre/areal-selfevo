...
#   step0i drops evaluator.freq_steps=20, which step0e/f/g all carried and which the
...
#
#   step0i DEVIATES from the demo config on purpose, and says so. step0e/f reproduced the
...
#          destroys held-out capability. step0i swaps in AReaL's own scaffolding values
...
#   step0i additionally sets evaluator.freq_epochs=null. Removing freq_steps in step0h was
...
RUN="$HOME/runs/step0i"; mkdir -p "$RUN"
LOG="$RUN/train.log"
...
exec 9>"$RUN/.lock"
flock -n 9 || { echo "a step0i run is already active in $RUN"; exit 3; }
...
python3 examples/math/gsm8k_rl.py \
  ...
  experiment_name=step0i trial_name=t1 2>&1 \
