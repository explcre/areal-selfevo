...
#   step0g DEVIATES from the demo config on purpose, and says so. step0e/f reproduced the
...
#          destroys held-out capability. step0g swaps in AReaL's own scaffolding values
...
RUN="$HOME/runs/step0g"; mkdir -p "$RUN"
LOG="$RUN/train.log"
...
exec 9>"$RUN/.lock"
flock -n 9 || { echo "a step0g run is already active in $RUN"; exit 3; }
...
python3 examples/math/gsm8k_rl.py \
  ...
  experiment_name=step0g trial_name=t1 2>&1 \
