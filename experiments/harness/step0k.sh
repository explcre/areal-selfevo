...
#   step0k drops evaluator.freq_steps=20, which step0e/f/g all carried and which the
#          headers called "one addition, which changes measurement and not training". It
#          changes training by HANGING it: py-spy caught step0g deadlocked at step 59 in
#          _evaluate -> rollout_controller.wait -> wait_results, every process alive and
#          all 8 GPUs at 0%. The reference sets evaluator.freq_steps: null; we restore it.
#          The evaluator`s signal is GSM8K eval reward -- the quantity already measured to
#          be directionless and mis-ordering w.r.t. held-out MATH-500 -- so repairing it
#          would buy nothing. Progress is measured by scoring saved checkpoints offline.
#
#   step0k DEVIATES from the demo config on purpose, and says so. step0e/f reproduced the
#          reference exactly; step0d ran it to 200 steps and its checkpoints were scored on
#          held-out MATH-500: 0.528 -> 0.316, monotone, while the GSM8K train reward
#          oscillated 0.209-0.721 with no trend. The demo recipe (lr 6e-6, eps_clip 0.4)
#          destroys held-out capability. step0k swaps in AReaL's own scaffolding values
#          (lr 1e-6, eps_clip 0.2) against that measured control, changing nothing else.
#          saver.freq_steps=25 is added because the reference saves NO checkpoints
#          (freq_steps: null), which makes a validation curve impossible.
...
#   step0k is the 7B arm of the routed experiment, run on the H200 box.
#
#   7B is chosen because it is the largest model with MEASURED headroom on the benchmark we
#   can actually score: MATH-500 is 0.734 at 7B (about 26 points of room) and 0.966 at 27B,
#   where no routing effect could be resolved. Scale beyond 7B needs a harder benchmark,
#   and OlympiadBench is the one with room (0.825 at 27B, 8% truncation).
#
#   Same corrected config as the 1.5B arm: adv_norm disabled, kl_ctl 0, group-aligned
#   microbatches. Those are not preferences -- they are the only settings under which the
#   rule's precondition holds, and the router refuses otherwise.
#
#   step0k is the FIRST run with token routing enabled.
#
#   Config is dictated by measurement, not preference. experiments/harness/measure_adv_norm.py
#   ran every adv_norm setting against the rule's precondition (sum_i A_i = 0 per group) on
#   unequal generation lengths: mean_level=group gives exactly 0.0000 at every std_level,
#   batch gives 0.81-1.23, and raw GRPO gives 2.00. So mean_level=group is required, and
#   kl_ctl drops to the reference default 0.0 because 0.01 breaks the same sum by 14-43%.
#   The router's guards refuse otherwise, so a misconfigured run cannot quietly proceed.
#
#   NOTE: without a teacher, teacher_logp is absent and the KD branch never runs, so this
#   arm tests only "withhold RL from provably dead tokens" -- not the full RL-to-teacher
#   redirect. That is a weaker claim and is labelled as such.
#
#   step0k additionally sets evaluator.freq_epochs=null. Removing freq_steps in step0h was
#          NOT enough: the reference also sets evaluator.freq_epochs: 1, so the epoch
#          trigger stayed live and step0h deadlocked at the same _evaluate_fn site at steps
#          87, 116 and 145 -- every 29 steps, which is every epoch at batch 256. Each firing
#          cost ~30 minutes or hung outright, so most of that run's wall clock was spent
#          stalled. Both triggers are now off, and freq_secs with them.
#
#   (evaluator.freq_steps=20 was here and has been REMOVED -- it deadlocked step0g at
#    step 59 inside _evaluate -> rollout_controller.wait. The validation curve it was meant
#    to provide is built instead by scoring saved checkpoints on held-out MATH-500, which
#    is a better signal anyway: the evaluator reports GSM8K eval reward, and GSM8K reward
#    was measured to be directionless and mis-ordering w.r.t. held-out accuracy.)
set -u -o pipefail
...
RUN="$HOME/runs/step0k"; mkdir -p "$RUN"
LOG="$RUN/train.log"
...
exec 9>"$RUN/.lock"
flock -n 9 || { echo "a step0k run is already active in $RUN"; exit 3; }
...
# The A100 box scrapes this key from an existing run script; this box has none, so one
# is generated on first use and reused. It authenticates the LOCAL rollout proxy only.
...
python3 examples/math/gsm8k_rl.py \
  ...
  experiment_name=step0k trial_name=t1 2>&1 \
