...
pg=$(cat "$HOME/runs/step0f/pgid" 2>/dev/null || echo "")
...
mkdir -p "$HOME/runs/step0f"
setsid bash "$H/step0f.sh" > "$HOME/runs/step0f/outer.log" 2>&1 < /dev/null &
sleep 3
pgid=$(ps -o pgid= -p $! 2>/dev/null | tr -d ' ')
echo "$pgid" > "$HOME/runs/step0f/pgid"
...
setsid bash "$H/watchdog.sh" "$HOME/runs/step0f/pgid" 1800 \
    "$HOME/runs/step0f/train.log" \
    "$HOME/areal-runs/logs/ubuntu/step0c/t1/main.log" \
    > "$HOME/runs/step0f/watchdog.log" 2>&1 < /dev/null &
