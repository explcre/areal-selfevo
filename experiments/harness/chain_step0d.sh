...
pg=$(cat "$HOME/runs/step0d/pgid" 2>/dev/null || echo "")
...
mkdir -p "$HOME/runs/step0d"
setsid bash "$H/step0d.sh" > "$HOME/runs/step0d/outer.log" 2>&1 < /dev/null &
sleep 3
pgid=$(ps -o pgid= -p $! 2>/dev/null | tr -d ' ')
echo "$pgid" > "$HOME/runs/step0d/pgid"
...
setsid bash "$H/watchdog.sh" "$HOME/runs/step0d/pgid" 1800 \
    "$HOME/runs/step0d/train.log" \
    "$HOME/areal-runs/logs/ubuntu/step0c/t1/main.log" \
    > "$HOME/runs/step0d/watchdog.log" 2>&1 < /dev/null &
