...
#   step0l drops evaluator.freq_steps=20, which step0e/f/g all carried and which the
...
#
#   step0l DEVIATES from the demo config on purpose, and says so. step0e/f reproduced the
...
#          destroys held-out capability. step0l swaps in AReaL's own scaffolding values
...
#   step0l is the ROUTING-OFF CONTROL for step0j, and the single most useful run missing
#   from the comparison.
#
#   step0j was measured against step0h and showed no effect (McNemar p 0.53-0.92), but
#   step0j differs from step0h in THREE ways: routing, kl_ctl (0.01 -> 0) and adv_norm
#   (batch -> off). Those had to move together because the routing rule's precondition does
#   not hold otherwise, so even a significant result could not have been attributed to
#   routing. This arm is step0j with the routing flags REMOVED and nothing else changed, so
#   step0l vs step0j isolates routing exactly.
#
#   A null result here is as informative as a positive one: it would confirm that the
#   distance from the demo recipe is entirely the optimiser settings.
#
...
#
#   step0l additionally sets evaluator.freq_epochs=null. Removing freq_steps in step0h was
...
RUN="$HOME/runs/step0l"; mkdir -p "$RUN"
LOG="$RUN/train.log"
...
exec 9>"$RUN/.lock"
flock -n 9 || { echo "a step0l run is already active in $RUN"; exit 3; }
