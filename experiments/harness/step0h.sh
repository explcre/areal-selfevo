...
#   step0h drops evaluator.freq_steps=20, which step0e/f/g all carried and which the
...
#
#   step0h DEVIATES from the demo config on purpose, and says so. step0e/f reproduced the
...
#          destroys held-out capability. step0h swaps in AReaL's own scaffolding values
...
RUN="$HOME/runs/step0h"; mkdir -p "$RUN"
LOG="$RUN/train.log"
...
exec 9>"$RUN/.lock"
flock -n 9 || { echo "a step0h run is already active in $RUN"; exit 3; }
...
python3 examples/math/gsm8k_rl.py \
  ...
  experiment_name=step0h trial_name=t1 2>&1 \
