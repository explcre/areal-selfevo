...
#   step0d changes NOTHING the reference sets. Both failures were our own deviations.
...
#
# THE EXPERIMENTAL VARIABLE, and the only one:
...
RUN="$HOME/runs/step0d"; mkdir -p "$RUN"
LOG="$RUN/train.log"
...
exec 9>"$RUN/.lock"
flock -n 9 || { echo "a step0d run is already active in $RUN"; exit 3; }
