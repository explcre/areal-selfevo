#!/usr/bin/env bash
# Wait for step0b to release the GPUs, then launch step0c with the fixed guards.
set -u
H="$HOME/areal-selfevo/experiments/harness"

# Wait for the step0b process group to exit (bounded).
pg=$(cat "$HOME/runs/step0b/pgid" 2>/dev/null || echo "")
...
mkdir -p "$HOME/runs/step0c"
setsid bash "$H/step0c.sh" > "$HOME/runs/step0c/outer.log" 2>&1 < /dev/null &
sleep 3
pgid=$(ps -o pgid= -p $! 2>/dev/null | tr -d ' ')
echo "$pgid" > "$HOME/runs/step0c/pgid"
...
setsid bash "$H/watchdog.sh" "$HOME/runs/step0c/pgid" 1800 \
    "$HOME/runs/step0c/train.log" \
    "$HOME/areal-runs/logs/ubuntu/step0c/t1/main.log" \
    > "$HOME/runs/step0c/watchdog.log" 2>&1 < /dev/null &
