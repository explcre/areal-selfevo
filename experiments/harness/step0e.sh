...
#   step0e changes NOTHING the reference sets. Both failures were our own deviations.
...
#   train_dataset.batch_size=128 + gconfig.n_samples=8 -- COMPUTE-NEUTRAL reallocation.
...
RUN="$HOME/runs/step0e"; mkdir -p "$RUN"
LOG="$RUN/train.log"
...
exec 9>"$RUN/.lock"
flock -n 9 || { echo "a step0e run is already active in $RUN"; exit 3; }
...
python3 examples/math/gsm8k_rl.py \
  ...
  experiment_name=step0e trial_name=t1 2>&1 \
