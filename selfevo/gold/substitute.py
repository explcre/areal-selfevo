...
from selfevo.gold.attach import GOLD_KEYS, GoldError, prompt_lengths
...
def substitute_gold_rows(
    ...
    *,
    ...
    Returns:
        ...
        int32 tensor that is 1 on every token of a gold row. Per TOKEN and not per row for the
    ...
    Raises:
        ...
        GoldPolicyError: For an unknown rule or policy name.
    """
    try:
        rule = GoldRule(rule)
    ...
    for key in GOLD_KEYS:
        if key not in batch:
            raise GoldMissingError(
                f"rule={rule.value} but the batch carries no {key}. The dataset adapter keeps "
                "the gold only when asked (keep_solution=True) and the workflow attaches it "
                "only when the dataset row carries it, so this means the gold arm is "
                "configured at one end and not the other."
            )

    sizes = _normalise_group_sizes(group_sizes, n_rows)
    width = int(batch["input_ids"].shape[1])
    for key in ("loss_mask", "attention_mask", "logprobs", *GOLD_KEYS):
        t = batch[key]
    ...
    for gi, size in enumerate(sizes):
        ...
        # by construction.
        victim = rows.start
        n_gold = int(gold_mask[victim].sum())
        if n_gold == 0:
            no_gold += 1
            continue
        p_len = int(prompt_len[victim])
        if p_len + n_gold > width:
            no_fit += 1
            continue
        _write_gold_row(
            out,
            row=victim,
            prompt_len=p_len,
            gold=batch["gold_ids"][victim, :n_gold],
            policy=policy,
    ...
    # that explain it. The empty-gold one is checked first because it is the more diagnostic
    # of the two: it says the supply failed, not the match.
    if int(gold_mask.sum()) == 0:
        raise GoldMissingError(
            f"rule={rule.value} but every gold_mask in this batch is empty, so there is no "
...
def substitute_in_place(
    ...
    **kwargs: Any,
    ...
    Args:
        ...
        **kwargs: Forwarded, except ``group_sizes``, which is fixed per element.
    ...
    for gi, traj in enumerate(trajectories):
        try:
            new, st = substitute_gold_rows(traj, rule, group_sizes=None, **kwargs)
