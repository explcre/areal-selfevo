...
MUTATIONS = [
    ...
    # ------------------------- the control's stability, and the roster the ids have to fit --
    #
    # The control had two of its three properties tested and the third, expert identity, not
    # at all: it re-permuted every batch, so it differed from the arm in stability as well as
    # in feature-blindness. Every mutation here breaks the carry in a way the size test and
    # the feature-blindness test both survive.
    (PARTITION, [T_PART], "the control memory is never consulted, so it re-permutes every batch",
     "            if remembered is not None and capacity[remembered] > 0:",
     "            if False:"),
    (PARTITION, [T_PART], "the carry ignores capacity, so a carried label breaks the size match",
     "            if remembered is not None and capacity[remembered] > 0:",
     "            if remembered is not None:"),
    (PARTITION, [T_PART], "nothing is ever remembered, so every batch is the control's first",
     "        for gid, label in zip(group_ids, out):\n            self._seen.pop(gid, None)\n            self._seen[gid] = int(label)",
     "        for gid, label in zip(group_ids, out):\n            pass"),
    (PARTITION, [T_PART], "the carry is skipped and the raw draw is returned",
     "        drawn = [int(v) for v in drawn]\n        if not group_ids:",
     "        drawn = [int(v) for v in drawn]\n        if True:"),
    (PARTITION, [T_PART], "each call gets a private memory, so nothing survives the batch",
     "    memory = DEFAULT_CONTROL_MEMORY if memory is None else memory",
     "    memory = MatchedControlMemory()"),
    (PARTITION, [T_PART, T_FEAT], "the control arm carries on the global memory, not the arm's",
    ...
    (PARTITION, [T_PART], "unseen groups are filled in sorted order, so the draw stops being seeded",
     "        for value in drawn:\n            if remaining[value] > 0:",
     "        for value in sorted(drawn):\n            if remaining[value] > 0:"),
    (PARTITION, [T_PART], "the control memory grows without bound",
     "        while len(self._seen) > self.max_entries:", "        while False:"),
    (PARTITION, [T_PART], "a control memory that remembers nothing is accepted",
     "        if max_entries < 1:", "        if False:"),
    (PARTITION, [T_PART], "batch positions are remembered when there are no identities",
     "        if not group_ids:\n            return tuple(drawn), 0",
     "        if not group_ids:\n            group_ids = [str(i) for i in range(len(drawn))]"),
    (PARTITION, [T_PART], "the roster bound never fires, so the expert ids stay unbounded",
     "            if self.max_experts is not None and self._next_stable >= self.max_experts:",
     "            if False:"),
    (PARTITION, [T_PART], "the roster bound is off by one, so it emits one expert too many",
     "            if self.max_experts is not None and self._next_stable >= self.max_experts:",
     "            if self.max_experts is not None and self._next_stable > self.max_experts:"),
    (PARTITION, [T_PART], "an overflowing cluster gets an id anyway rather than the shared adapter",
     "                self._overflow.add(raw)\n                return -1",
     "                self._overflow.add(raw)"),
    (PARTITION, [T_PART], "the clusters the roster turned away are not recorded",
     "                self._overflow.add(raw)\n                return -1",
     "                return -1"),
    (PARTITION, [T_PART], "the overflow set is never rebuilt, so it reports the run's history",
     "        self._overflow.clear()\n", "\n"),
    (PARTITION, [T_PART], "the basis says nothing about the clusters folded into shared",
     "        if self._overflow:\n            # Present tense",
     "        if False:\n            # Present tense"),
    (PARTITION, [T_PART], "a max_experts of zero is accepted at construction",
     "        if max_experts is not None and int(max_experts) < 1:", "        if False:"),
    (PARTITION, [T_PART], "the bound is dropped, so a configured roster does not bound anything",
     "        self.max_experts = None if max_experts is None else int(max_experts)",
     "        self.max_experts = None"),
    (PARTITION, [T_PART], "a roster with no cluster_<i> expert is accepted",
     "    if not numbers:", "    if False:"),
    (PARTITION, [T_PART], "a roster whose expert numbers have a gap is accepted",
     "    if numbers != list(range(len(numbers))):", "    if False:"),
    (PARTITION, [T_PART], "a roster with no shared adapter is accepted",
     "    if SHARED_CLUSTER not in names:", "    if False:"),
    (PARTITION, [T_PART], "the bound counts every roster entry, not just the experts",
     "    return len(numbers)", "    return len(names)"),
    # ------------------------------------------------------------------------ the sketch --
    ...
    # --------------------------------- the full-gradient store and what it is selected from --
    (DUMP, [T_PROBE], "the store takes the first N groups, so on a silent batch it stores zeros",
     "    informative = [i for i, g in enumerate(groups) if not g.is_silent]",
     "    informative = [i for i, g in enumerate(groups)]"),
    (DUMP, [T_PROBE], "the store ignores its selection and takes file order anyway",
     "    full_idx = {i: k for k, i in enumerate(informative[:n_full])}",
     "    full_idx = {i: k for k, i in enumerate(range(n_full))}"),
    (DUMP, [T_PROBE], "no group is ever silent, so the selection cannot avoid the zeros",
     "        return bool(r.std() == 0.0)", "        return False"),
    (DUMP, [T_PROBE], "an unvalidatable store is reported as fine",
     "    if n_nonzero_full >= 2:", "    if True:"),
    (DUMP, [T_PROBE], "the store is upcast on the card before it is moved to the host",
    ...
    # ------------------------- the guard's INPUT: the caching allocator's reserved blocks ----
    (DUMP, [T_PROBE], "the cache is never released, so the guard reads the allocator's history",
     "            if release_cache:\n                torch.cuda.empty_cache()",
     "            if False:\n                torch.cuda.empty_cache()"),
    (DUMP, [T_PROBE], "the cache is released AFTER the read, which measures nothing",
     "            if release_cache:\n                torch.cuda.empty_cache()\n            return int(torch.cuda.mem_get_info(torch.device(device))[0])",
     "            got = int(torch.cuda.mem_get_info(torch.device(device))[0])\n            if release_cache:\n                torch.cuda.empty_cache()\n            return got"),
    (DUMP, [T_PROBE], "the budget flag defaults to a value again, so the derived branch never runs",
     '    p.add_argument("--logits-budget-gb", type=float, default=None,',
     '    p.add_argument("--logits-budget-gb", type=float, default=4.0,'),
    (DUMP, [T_PROBE], "the count of gradient-free groups is reported as zero",
     "    n_zero_sketch = sum(1 for v in sketches if float(np.abs(v).max()) == 0.0)",
     "    n_zero_sketch = 0"),
    (ANALYZE, [T_ANALYZE], "the analysis reports no group as gradient-free",
     '        "n_zero_grpo_sketches": int(sum(1 for v in sketches if float(np.abs(v).max()) == 0.0)),',
     '        "n_zero_grpo_sketches": 0,'),
    (ANALYZE, [T_ANALYZE], "a dump with no stored gradients reports the sketch as validated",
     '            "status": "IMPOSSIBLE",\n            "note": "no full gradients in the dump',
     '            "status": "ok",\n            "note": "no full gradients in the dump'),
    # ---------------------------------------------------------------------- the analysis --
