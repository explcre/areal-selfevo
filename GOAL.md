...
| M25 | **MEDS-routed LoRA experts** (the METHOD paper, PI decision 2026-09-01) | **BUILDING** | Per-cluster LoRA adapters during GRPO: groups clustered by behaviour (MEDS: HDBSCAN over latter-half layer logits at the answer token, kNN-stabilised across batches), each group's loss updates only its cluster's adapter, HDBSCAN noise -> shared adapter, adapters MERGED at inference (LSPO-style, 2607.27787). Hypothesis with a mechanism: behavioural subpopulations need conflicting updates; a shared adapter averages them (gradient interference); per-cluster adapters remove it. **Mandatory control**: size-matched random partition (same N, same sizes, feature-blind labels) — separates "clustering" from "more adapters". **Mechanistic evidence first**: `interference_probe.py` on existing checkpoints (per-cluster gradient cosine, conflict rate, cancellation) vs the random partition — minutes on one H100, no training; if the random control shows the same conflict, the clusters are noise. Baselines to beat at matched budget: shared LoRA (vanilla), LSPO (2-expert by k), random-matched partition, task-label LoRAs. Benchmarks: OlympiadBench + LiveCodeBench at 32B. Known risks: MEDS clusters RESPONSES so prompt-time routing is circular (merge sidesteps it — benefit is training-time); cluster relabelling churn; cluster-then-merge is adjacent to task arithmetic (TIES/DARE) — novelty is discovered clusters during RL. Novelty search pending (`fable-findings0901.md` §6.4) |
| M26 | **LLM-EDITS-THE-POLICY (code-as-policy evolution)** — the LLM rewrites the routing SOURCE between segments, not the decision per group | **DESIGN FIXED 2026-09-02 (PI); executor DONE, generator NOT BUILT** | **The cost profile is why this is the viable form.** M23 needs one LLM call per group per batch (64/step, forever); this needs ONE call per evolution step, and the compiled policy then routes every group for free — amortised over a whole segment. **Already built**: `CodePolicyRouter` executes generated source safely (AST allowlist, subprocess cost-vetting, ~150 adversarial policies, 26/26 mutants, `PolicyRejected` at CONSTRUCTION so a bad policy never reaches a batch), and `actor.py:610` feeds it the seven observability features. **Missing**: the generator, and a config path for the source (being wired as `router_source_file`). **The generator is free** — the 32B already served for rollouts writes the source, so no API spend and no permission needed. **Three constraints, each measured here today.** (1) The evidence handed to the LLM must be PER-PROMPT, never a batch scalar: the credit lemma applies to an LLM reading outcomes exactly as to a bandit, and a shared scalar carries zero bits about which decision earned what — that is why Result 7's null was inevitable. (2) It needs a MATCHED CONTROL: same cadence and number of policy changes, proposals that ignore the evidence, or "the policy evolved" is confounded with "the policy changed". (3) It must be gated on `r_covariate`: if no covariate carries signal beyond the pass count, a generator has nothing to find and will produce policies that are elaborate restatements of a solve-rate threshold. **Build order: `r_covariate` -> generator -> matched control.** |
| M23 | **LLM-as-router**: an LLM reads a unit's rollouts (failure modes, solve rate, observability metrics) and decides evolve-model vs evolve-harness | **NOT BUILT — design fixed 2026-09-01 (PI)** | **Cheapest good version first: FROZEN LLM + retrieved experience + the rule as prior.** No router training. The router prompt carries (i) the hand-written rule (DyME+DAPO = our own rule policy) as the cold-start prior the LLM may deviate from, (ii) the unit's current features and a trajectory excerpt (the covariate a threshold cannot read), and (iii) top-k RETRIEVED experiences from a store of (features, decision, **per-prompt credit delta**) triples from this run, retrieved by feature similarity so the prompt stays bounded and prefix-cacheable. This is a non-parametric learned router: it updates instantly, costs inference only, and its "learning" is the store. **Non-negotiable**: the stored outcome must be per-prompt credit across time (SPO 2509.13232 / PAC 2608.30528), never the batch scalar — the credit lemma applies to an experience store exactly as to a bandit: a shared scalar in the store carries zero bits about the decision. **LoRA-trained router = the later arm**, after the frozen+experience router sets the bar; its cost is backprop through a router LLM per update plus a meta-RL signal, and GOAL.md's build order puts the trained LLM router last. Controls unchanged: rate-matched random at the router's realised proportions; subset contrast + shuffle; matched inference budget (the LLM call per group per batch must be counted). Relation to M25: M25 routes WHICH ADAPTER receives an update (parameter space, by MEDS cluster — the method); M23 routes evolve-target / loss / source (the ablation). Build M23 after the M25 mechanism and the interference probe report. | See the positioning note below -- the idea is a combination of existing lines, so the contribution has to be the comparison, not the idea |
...
|---|---|---|---|
| **SIA** (2605.27276) | the **baseline to beat**, and the origin of our framing — VERIFIED 2026-09-01: a frozen prompted LLM ("Feedback-Agent") decides harness-vs-weights AND which RL algorithm per ITERATION from trajectory observations (not task-level); its §9 states "meta-RL over the action-selection policy" as future work in so many words. We must cite it as the origin and beat its frozen-LLM selector head-to-head at matched budget | `hexo-ai/sia` | **NOT CLONED** |
| **MEDS** (2604.11297) | a concrete **`key_fn` for `ClusterRouter`**: reuses **layer-wise logits** (latter-half layers, final answer token) as lightweight behaviour representations and clusters **error patterns** with **HDBSCAN**. VERIFIED 2026-09-01: actual title is "The Past Is Not Past: Memory-Enhanced Dynamic Reward Shaping"; it is anti-repetition reward shaping `r̃ = r − min(α log(|C_k|+1), β)` and contains NO routing — reuse the feature, do not cite it as routing support. `cluster_method=hdbscan`, `use_layer_diff`; actor-side logic in `verl/workers/actor/dp_actor.py` | on disk at `~/baselines/MEDS` | **PRESENT, unused** |
| **Co-Harness** (2607.22688) | prior art for success→model / failure→harness — VERIFIED 2026-09-01: the ONE genuine fixed rule among our comparisons (deterministic verifier split + LLM critic inside a fixed taxonomy; model update is plain SFT, T=2 rounds). Our extension is that one trajectory can feed **both**, decided by routing | — | **NOT CLONED** |
| **EvoTrainer** (2606.03108) | co-evolves policy **and training harness**; closest to our evolve-target axis — VERIFIED 2026-09-01: decided by a frozen Claude Sonnet 4.6 at training-VERSION granularity, human-gated (§3.5); already reads dead-group ratio, within-group reward std and truncation rate as decision features and selects GRPO/GSPO cores. Not a fixed rule | — | **NOT CLONED** |
| **Learning Fast & Slow** (2605.12484) | 3x sample efficiency and 70% less KL-from-base are exact — but VERIFIED 2026-09-01 that it contains NO cadence analysis or bounds; it is a continual-learning/plasticity paper with both channels always on. Do not cite it for cadence | — | reference only |
...
**RESOLVED 2026-09-02: the DeepMath switch more than doubled the effective batch.** A0's first checkpoint on decontaminated DeepMath-103K, 128 groups from the live run (steps 9-24): `k=0:22 k=1:10 k=2:5 k=3:5 k=4:9 k=5:9 k=6:12 k=7:19 k=8:37` -> **69/128 informative = 53.9%**, against 23.3% on MATH; fully-correct groups 69.9% -> 28.9%. Roughly 35 of 64 rollouts per step now carry gradient instead of 15, and the distribution is spread across every k rather than piled at the ends, so MEDS will partition over ~69 informative groups per 128 rather than 31. Still short of the >=60% the PROBE precondition requires, so Gate 0 batches still need over-sampling and filtering, but the training signal itself is healthy. **Watch the cap**: 11.5% of responses now hit `max_new_tokens=1024` (5.8% on MATH) because DeepMath answers are longer — not enough to be manufacturing the k=0 groups, but closer to that regime than MATH was. Rate ~30 s/step (22.9 on MATH).

**DECONTAMINATION — the first scan was the weaker one, and the correction matters.** The word-5-gram scan reported earlier **missed one of the two verbatim pairs it was meant to catch** (row 97406 <-> OlympiadBench idx 395). The two strings differ only in `\cos ^{3}` vs `\cos^3`, `$...$` vs `\[...\]` and whitespace; splitting on whitespace lines up almost no 5-grams, and an 88-character problem has only ~9 grams, so a single formatting difference sinks the metric. A second pass now normalises format away (lowercase; delete whitespace, `${}[]()` and sentence punctuation; **keep operators**) and compares character-25-grams, under which those two strings are identical. A row is dropped if EITHER pass flags it: **187 of 103,022 dropped (0.18%)**, 102,835 kept, audit with per-row scores at `~/data/deepmath_decontam/dropped_rows.json`. Both verbatim pairs are dropped and 7 of the 8 eval indices at >=0.5 are implicated. **One honest disagreement remains**: idx 28 was scanned against all 103,022 rows individually and its best word containment is 0.111 and best char containment 0.083 — no row contains it at 0.3 under either metric. The looser scan that flagged it may be matching generic phrasing; treat idx 28 as UNRESOLVED rather than clean or contaminated.

**MEASURED 2026-09-02 ON THE BASELINE'S OWN TRAINING BATCHES: 77% of A0's groups are RL-silent.** Sampled across steps 13-24 of the A0 run itself (not a probe batch): k histogram `k=0:9 k=1:5 k=2:2 k=3:4 k=4:6 k=6:7 k=7:7 k=8:93`, so **102/133 unanimous, 31/133 informative (23.3%)**. Under GRPO a unanimous group has every advantage identically zero, so **A0's effective batch is ~23% of its nominal size** — of 64 rollouts per step, roughly 15 carry gradient, and the other 49 are paid for in full. Worse than either probe batch (Level 5 filtered 100%, Level 5 raw 39.8%, Level 4-5 raw 29.7%) because A0 trains the FULL MATH split, all levels, where 32B is saturated. **Consequence for the arms**: a partition over 133 groups of which 102 have zero gradient is a partition over 31 groups, so MEDS would assign experts largely on the basis of groups that cannot train them. This is the measurement that moved A0 to DeepMath-103K.
...
**STAGED 2026-09-02: `zwhe99/DeepMath-103K`** (arXiv 2504.11456, **MIT**, revision `5cf055d1fe3d7a2eb19719ac020211469736ae44`), verified **103,022 rows** by summed parquet `num_rows`, 2,136 MB, 0 `*.incomplete`. Registry key `deepmath`. Chosen because **LSPO trains on it** (confirmed: whole corpus, no filtering stated, on DeepSeek-R1-Distill-Qwen-1.5B), so arm A4 is corpus-matched. Difficulty 1.0-10.0, median **6.0**, 86.2% at >=5.0, against MATH's Level 5 = 30.7%.

**CONTAMINATION — DECLARED, and it contradicts the dataset card.** DeepMath's paper claims decontamination against OlympiadBench by embedding retrieval plus an LLM judge. A word-5-gram containment scan after normalisation still finds **2 of 675 OlympiadBench problems verbatim** (idx 395 -> row 97406, idx 517 -> row 100455), **8 of 675 at >=0.5** (also idx 326, 544, 462, 346, 459, 28) and 35 at >=0.3. That is ~1.2% of the benchmark we report on — below its 2.7-point noise floor, but **semantic decontamination demonstrably did not catch verbatim duplicates**, so it must be declared and the matched rows removed rather than argued away. **AIME 2025/2026 are outside DeepMath's decontamination scope entirely.** Against LiveCodeBench v6: 0 exact matches, max containment 0.090, all spurious — but **provenance is UNVERIFIED**, since no cutoff date is stated anywhere and DeepMath's decontamination covers no code benchmark at all.

**The gold path changes shape on this corpus.** DeepMath's derivations are R1 traces: solution tokens median **4,369** / p90 **10,160**, against MATH's 162 / 482. Only **0.34% fit a 1024 cap** (MATH: 99.15%); 84.4% fit 8,192. So on DeepMath the gold supplier's reach is bounded by the GENERATION CAP through `groups_no_fit`, not by group composition — the opposite of MATH. The gold template also inverts: R1 traces carry exactly one `</think>` in 99.999% of rows and no `<think>` in 99.965%, so MATH's template would splice a second close mid-turn (guarded).

**A tempting invalid shortcut, refused and recorded.** DeepMath ships three R1 samples per row, which looks like a free CPU k-histogram — graded with our own reward it gives k=0 at 0.10% and k=3 at 99.57%. **That number is circular**: the corpus was FILTERED on those very samples during construction, so it measures the filter, not the difficulty. **Do not cite it.** A real k-histogram needs a GPU rollout on our own policy.

**Code corpora (survey only, nothing staged).** `open-r1/codeforces` and `BAAI/TACO` are the only candidates whose pre-window provenance is **provable per row** (`contest_start` / `date` fields), which is what a LiveCodeBench v6 claim requires. Avoid `nvidia/OpenCodeReasoning` and the Skywork code split. Skywork-OR1 declares **no licence** and is not vendorable; OpenR1-Math-220k is the only surveyed corpus shipping a runnable decontamination script.
